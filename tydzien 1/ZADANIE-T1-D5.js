...
*/
// Miejsce na Twój kod.
function pociong(ciong) {
    return ciong.reverse();
};
...
function sprawdzam(jeden, dwa) {
    return JSON.stringify(jeden) === JSON.stringify(dwa);
};
...
function inzynieriaOdtylcowa(majtki) {
    return majtki.split('').reverse().join('');
};
...
function pascal(obiekt) {
    return Object.values(obiekt)[0];
}
...
function xNzq(stronyKsiazki) {
    const unikalneKsiazki = Array.from(new Set(stronyKsiazki.map(obj => obj.strona)));
    
    unikalneKsiazki.sort((a, b) => a - b);

    return unikalneKsiazki.map(strona => {
        return stronyKsiazki.find(obj => obj.strona === strona);
    });;
};
...
function detektor(dlugiCiag, wzor) {
    return dlugiCiag.split(wzor).length - 1;
}
...
function mors(kodMorsa) {
    ...
    return kodMorsa.split(' / ').map(slowo => {
        return slowo.split(' ').map(letter => alfabetMorsa[letter] || ' ').join('');
    }).join(' ');
};
